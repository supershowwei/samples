...
using (var csv = new CsvReader(reader, CultureInfo.InvariantCulture))
{
    ...
    foreach (var candlestick in candlesticks)
    {
        ...
        {
            if (candlestick.FromDate.DayOfWeek == DayOfWeek.Monday)
            {
        ...
        {
            if (weeklyOptionCandlestick?.FromDate < candlestick.FromDate && candlestick.FromDate.Subtract(weeklyOptionCandlestick.FromDate).TotalDays <= 2)
            {
