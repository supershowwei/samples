SELECT [r].[scheduler_id] AS 排程器識別碼,
       ...
       SUBSTRING(LTRIM([q].text), [r].[statement_start_offset]/2+1, (CASE
                                                                     ...
                                                                     END-[r].[statement_start_offset])/2) AS [正在執行的 T-SQL 命令],
...
FROM [sys].[dm_exec_requests] [r]
     CROSS APPLY [sys].[dm_exec_sql_text]([sql_handle]) AS [q] LEFT JOIN [sys].[databases] [d] ON([r].[database_id] = [d].[database_id])
